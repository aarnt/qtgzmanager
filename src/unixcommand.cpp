...
QString UnixCommand::discoverBinaryPath(const QString& binary){
  ...
  QString sParam = QLatin1String("which ") + binary;
  QStringList sl;
  sl << QLatin1String("-c");
  sl << sParam;

  proc->start("/bin/sh", sl);
...
bool UnixCommand::isSlackPackage(const QString& filePath){
  ...
  if (result == ""){
    ...
    if (result == "")
      bResult = false;
    else if (result.contains("install/") || result.contains("not found", Qt::CaseInsensitive))
      bResult = true;
  }
  else
    bResult = true;
...
QByteArray UnixCommand::getPackageContents(const QString& pkgName){
  ...
  if (res == ctn_XZ_TAR_ERROR || res == ctn_GZ_TAR_ERROR) return ctn_PKG_CONTENT_ERROR.toLatin1();
