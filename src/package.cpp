...
QString Package::getInformation( QString pkgName, bool installed ) {
    ...
    while(x >= 0){
        ...
        if (!sb.midRef(x-1, x).contains("\\S")) sb.remove(x, (x+t.length()+1)-x);
    ...
    if (a == 0){
      ...
      if ((c.size() != 0) && (d.size() != 0) && (c.size() == d.size())){
        ...
        if (goAhead){
          ...
          int versionTest = rpmvercmp(st1.toLatin1().data(), st2.toLatin1().data());
          ...
          if (versionTest == 1)
            ...
            int releaseTest = rpmvercmp(sn1.toLatin1().data(), sn2.toLatin1().data());
