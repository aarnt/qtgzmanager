...
void PackageController::testSearchInDir(){
  QMultiMap<QString, QStringList> map = findPackage("qt-", "/home/arnt/Packages");

  for (QString k: map.keys()){
    for( QStringList sl: map.values(k) ){
      for ( int c=0; c<sl.count(); c++ ){
        QString found(sl[c]);
        std::cout << "Found!" << k.toLatin1().data() << "/" << found.toLatin1().data() << std::endl;
      }
    }
  }
}
